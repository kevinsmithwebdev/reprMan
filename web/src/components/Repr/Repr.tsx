  ...
  return (
    <Card text='dark' className='mb-2' style={{
      ...
      <Card.Body>
        ...
        <Card.Subtitle>
          Last Practiced: {lastPracticedMoment.format('MMMM Do YYYY, h:mm a')}, {lastPracticedMoment.fromNow()}
        </Card.Subtitle>
      </Card.Body>
    </Card>
  )
...
const getReprColors = (lastPracticed: number, daysOverdueTrigger: number): ReprColor => {
  const daysAgo = moment().diff(lastPracticed, 'days')

  if (daysAgo > daysOverdueTrigger) return {bg: '#fff6f6', border: 'danger'}
  if (daysAgo > daysOverdueTrigger * WARN_VALUE) return {bg: '#fef9e4', border: 'warning'}
